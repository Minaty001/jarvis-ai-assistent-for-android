...
_INTENT_PATTERNS: list[tuple[str, str, list[str]]] = [
    ...
    ("take_screenshot", r"^(?:capture\s+(?:the\s+)?screen|screen\s+(?:capture|shot))$", []),
    ("send_notification", r"^(?:send|post|push)\s+(?:a\s+)?notification\s+(?:saying|that\s+says|with\s+content)\s+(.+)", ["content"]),
    ("send_notification", r"^notify\s+me\s+(?:that|about|saying)\s+(.+)", ["content"]),
    ("airplane_mode", r"^(?:turn\s+on|enable|switch\s+on)\s+(?:airplane|flight)\s+mode$", []),
    ("airplane_mode", r"^(?:turn\s+off|disable|switch\s+off)\s+(?:airplane|flight)\s+mode$", []),
    ("airplane_mode", r"^(?:toggle|switch)\s+(?:airplane|flight)\s+mode$", []),
    ("do_not_disturb", r"^(?:turn\s+on|enable|switch\s+on)\s+(?:do\s+not\s+disturb|dnd|silent\s+mode)$", []),
    ("do_not_disturb", r"^(?:turn\s+off|disable|switch\s+off)\s+(?:do\s+not\s+disturb|dnd|silent\s+mode)$", []),
    ("do_not_disturb", r"^(?:toggle|switch)\s+(?:do\s+not\s+disturb|dnd|silent\s+mode)$", []),
    ("sensor_data", r"^(?:read|get|check|list)\s+sensor(?:\s+data)?(?:\s+(.+))?", ["sensor"]),
    ("sensor_data", r"^sensor\s+(?:readings?|telemetry|status)(?:\s+(.+))?", ["sensor"]),
    ...
    ("set_timer", r"^(?:set|start|create)\s+(?:a\s+)?recurring\s+timer\s+(?:for\s+)?(\d+)\s*(seconds?|secs?|minutes?|mins?)(?:\s+(?:for|called|labeled)\s+(.+))?$", ["duration", "unit", "label"]),
    ("set_timer", r"^(?:repeat|repeat\s+every)\s+(\d+)\s*(seconds?|secs?|minutes?|mins?)(?:\s+(?:for|called|labeled)\s+(.+))?$", ["duration", "unit", "label"]),
    ...
    ("tell_time", r"^(?:what(?:\'s|\s+is)\s+)?(?:the\s+)?\btime\b(?:\s+now)?$", []),
    ("tell_date", r"^(?:what(?:\'s|\s+is)\s+)?(?:the\s+)?\bdate\b(?:\s+today)?$", []),
    ...
    ("export_conversation", r"^(?:export|save|download|backup)\s+(?:chat|conversation|history)$", []),
    ("export_conversation", r"^(?:export|save|download|backup)\s+(?:the\s+)?(?:chat|conversation)\s+(?:history|log)$", []),
    ("search_conversation", r"^(?:search|find|look\s+up)\s+(?:in\s+)?(?:my\s+)?(?:conversation|chat|history)\s+(?:for\s+|about\s+)?(.+)", ["query"]),
    ("search_conversation", r"^what\s+did\s+(?:we\s+)?(?:talk|say|discuss)\s+(?:about\s+)?(.+)", ["query"]),
    ...
    ("what_is", r"^what(?:\'s|\s+is)\s+(?:my\s+|the\s+)?(.+)", ["query"]),
    ("who_created", r"^(?:who\s+(?:is\s+your\s+|owns\s+)?(?:created|made|built|build|owner|father|creator|maker)(?:\s+you|\s+jarvis)?|who\s+made\s+you|who\s+created\s+you|who\s+built\s+you|who\s+is\s+your\s+creator|who\s+is\s+your\s+maker)\??$", []),
    ("exit", r"^(?:exit|quit|goodbye|bye|shutdown|see\s+you)\s*$", []),
]
