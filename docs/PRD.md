...
- Runs locally (STT optional offline via Vosk)
- Provides LLM-powered reasoning via an API of choice (Groq, OpenAI-compatible)
...
- Offline speech-to-text via Vosk (local model)
- LLM-powered chat and reasoning via Groq API (llama3-8b-8192)
...
- No Vosk → text-only input
- No Groq key → informative error
...
- No wake-word model training — uses simple keyword-spotting on Vosk partial results
- No cloud STT — all speech recognition is local (Vosk), no plan to add cloud STT
...
| STT → response latency | <3s with local STT + Groq LLM |
| Tests passing | 51 / 51 |
| Graceful degradation scenarios | All 6 failure modes handled |
